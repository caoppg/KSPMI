 ...
 * on a hash map of entries <Integer, List {@link Position>}, and it makes a
 ...
 * SPMF. If not, see  {@link http://www.gnu.org/licenses/}.
 *
 * @author agomariz
 */
public class IDListStandard_Map implements IDList {
    ...
    /**
     * It creates an IdList from a map of <Integer,List {@link Position>}
     *
...
int size = (((IDListStandard_Map) idList).getSequencePositionsEntries().size() > ((IDListStandard_Map) this).getSequencePositionsEntries().size()) 
	? ((IDListStandard_Map) this).getSequencePositionsEntries().size() : ((IDListStandard_Map) idList).getSequencePositionsEntries().size();
 ...
 * Tin modifies:        
 */
if (!twoFirstEventsEqual) dif[0] += (originalSizeOfSequences.get(key) - eid.getItemIndex());                             
//                            dif[0] += (originalSizeOfSequences.get(key) - eid.getItemIndex());
                        } else {
                            result.add(currentPosition);
if (!twoFirstEventsEqual) dif[0] += (originalSizeOfSequences.get(key) - currentPosition.getItemIndex());                             
