...
/**
 * Implementation of pattern structure. We define it as a list of pairs  {@link abstraction, item}.
...
public class Pattern implements Comparable<Pattern> {
    ...
    /**
     * List of pairs  {@link abstraction, item} that define the pattern
     */
    ...
    /**
     * New pattern from a list of pairs  {@link abstraction, item}
     * @param elements 
     */
    ...
    /**
     * New pattern from a single pair  {@link abstraction, item}
     * @param pair 
     */
    ...
    /**
     * Transform this pattern to a {@link ca.pfv.spmf.algorithms.sequentialpatterns.clasp_AGP.dataStructures.Sequence} 
     * object in order to perform sequential operations easier.
     * @return A new sequence object representing this pattern.
     */
    public Sequence toSequence() {
        Sequence seq = new Sequence(0);
        int indexItemset = -1;
        for (ItemAbstractionPair iap : elements) {
            if (!((Abstraction_Qualitative) iap.getAbstraction()).hasEqualRelation()) {
                seq.addItemset(new Itemset());
                indexItemset++;
            }
            seq.addItem(indexItemset, iap.getItem());
        }
        return seq;
    }
}
